...
% ===========================================================================
\section{Headline 2: masterpieces are unlike everything else}
% ===========================================================================

\begin{frame}{Same machinery, new grouping: transitions by problem type}
  \begin{columns}[T]
    \column{0.52\textwidth}
    \begin{center}
      \includegraphics[height=0.72\textheight]{similarity_types_symmetric.png}
    \end{center}
    \column{0.44\textwidth}
    \begin{itemize}
      \item Group the same 4{,}043 chains by Claude's \emph{Quarfoot type} instead of
            by domain, and recompute the similarity matrix
      \item Every pair of non-masterpiece types: similarity $\geq \mathbf{0.89}$
      \item Masterpieces vs.\ everything: \textbf{0.73--0.82} (mean 0.77)
    \end{itemize}
    \medskip
    Domains differ by a few percent. Masterpieces sit \emph{outside the family} ---
    the largest effect anywhere in this study.
  \end{columns}
\end{frame}

\begin{frame}{What makes a masterpiece different?}
  \begin{columns}[T]
    \column{0.44\textwidth}
    Raw transition counts, all masterpiece chains:
    \begin{center}
      \small
      \begin{tabular}{l|cccc}
        ...
        $r$ & 0 & 0 & 1 & 0 \\
        $b$ & 0 & 0 & \textbf{5} & 2 \\
        $e$ & \bad{0} & \bad{0} & \bad{0} & \bad{0} \\
      \end{tabular}
    \end{center}
    \medskip
    \footnotesize
    \bad{Caveat:} that is 2 chains, 12 hints --- Claude finds only \textbf{2
    masterpieces in all 4{,}043 MATH problems}. Read the direction, not the digits.
    \column{0.52\textwidth}
    \begin{itemize}
      \item \textbf{The extension coda vanishes.} Every other type's chains settle into
            extensions at the end ($e\to e$ from $0.63$ to $0.83$); masterpiece chains
            never repeat an extension --- \emph{the problem is the extension}
      \item \textbf{Bottom-out carries the chain}: 7 of 10 transitions start at $b$ ---
            the hints construct the result step by step
      \item \textbf{The endpoint of the substance axis.} Moving FN $\to$ SP,
            $b\to b$ climbs $0.56 \to 0.78$ and the principle-stating share falls
            $31\% \to 13\%$; masterpieces push both further
      \item \textbf{And they're vanishingly rare here} --- so rare a competition item
            bank can't even estimate their dynamics. Where \emph{do} they live?
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Reading the tails as pedagogy}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \bad{Zero Masterpieces} --- headline 2 in corpus form
...
\begin{frame}{Takeaways, caveats, next steps}
  ...
  \begin{enumerate}
    ...
    \item Masterpieces' hint dynamics look like no other type's --- and MATH barely
          contains them (2 of 4{,}043); capstones live in curated texts, not item banks
  ...
  \begin{itemize}
    ...
    \item The masterpiece transition matrix rests on 2 chains --- the outlier direction
          is consistent, but the estimate is prior-dominated
  ...
  generate Socratic chains for the book's 40 Masterpieces to test headline 2 at real
  sample size; hint transitions vs.\ difficulty level; HMM over noisy labels.
\end{frame}
